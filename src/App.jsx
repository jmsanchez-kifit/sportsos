...
export default function SportOS() {
  ...
  return (
    <div style={ss.wrap} data-sport={sport}>
      <AuroraBg/>
      ...
      {isDemo ? (
        <div style={{background:"linear-gradient(90deg,rgba(168,85,247,0.15),rgba(59,130,246,0.15))",borderBottom:"1px solid rgba(168,85,247,0.25)",padding:"6px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:"11px",gap:"8px",flexWrap:"wrap"}}>
          <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
            ...
            <span style={{color:"var(--text-3)"}}>— puedes cambiar entre roles libremente.</span>
          </div>
      ...
      <div style={ss.topbar}>
        ...
        {(isDemo || role !== "jugador") && <>
          <div style={{display:"flex",gap:"2px",background:"var(--bg-elev-2)",borderRadius:"var(--r-md)",padding:"3px",overflowX:"auto"}}>
          ...
          <select value={category} onChange={e=>setCategory(Number(e.target.value))} style={{...ss.input,width:"100px",fontSize:"12px",padding:"6px 10px",cursor:"pointer"}}>
            {sp.categories.map((c,i)=><option key={i} value={i}>{c}</option>)}
          </select>
        </>}
        <div style={{flex:1}}/>
        <div style={{fontSize:"11px",color:"var(--text-2)",display:"flex",alignItems:"center",gap:"4px",padding:"5px 10px",background:"var(--bg-elev-2)",borderRadius:"99px",whiteSpace:"nowrap"}}>🇨🇱 CLP</div>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        {/* Sidebar */}
        <motion.div {...fadeUp} style={ss.sidebar}>
          <div style={{padding:"18px 14px",borderBottom:"1px solid var(--border-soft)",textAlign:"center"}}>
          ...
          <div style={{padding:"12px 8px 4px",borderTop:isDemo?"1px solid var(--border-soft)":"none",marginTop:isDemo?"6px":"0",flex:1}}>
        ...
        {/* Main content */}
        <div style={ss.main} key={role+module}>
